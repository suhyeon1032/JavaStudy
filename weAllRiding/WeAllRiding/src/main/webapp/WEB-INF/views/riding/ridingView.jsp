<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<link href="${url}/css/riding/ridingList.css" rel="stylesheet"
	type="text/css">
...
<script type="text/javascript">
	...
	var membertest = $("nicknameTest").val();
	var param = {"nicknameTest":}
	...
	function ridingReplyListAll() { //현재글의 댓글을 모두 가져오기
		...
		$.ajax({
			...
			success : function(result) {
				...
				$result.each(function(idx, vo) {
					tag += "<li><div id='dddd' style='color:black;'><div id='NN'>"
					+ vo.nickname + "</div>";
					// 	 'goguma'== goguma
					if (vo.nickname == '${nickName}') {
						tag += "<input type='button' value='삭제' id='ridingReplyListDel' title='"
							+ vo.ridingReplyNo
							+ "' onclick='ridingReplyListDel()' />";
						tag += "<input type='button' value='수정' id='ridingReplyListEdit'/>";
					}
						tag += "<br/><div>" + vo.ridingReplyComent + "</div>";
						tag += "<div id='CRWD' style='color:lightgray;'>"
						+ vo.ridingReplyWriteDate
						+ "</div></div>";
		...
		$(document).on('click', '#ridingReplyList input[value=수정]', function() {
	...
	<div class="ridingViewContainer">
		<form method='post'>
			<ul>
				...
				<br>
				<br>
				<h2 id="ridingViewTitle">
					<input type="hidden" name="ridingSubject"
						value="${vo.ridingSubject }">${vo.ridingSubject }</h2>
				...
				<li id="dateAll" style="color: black;">
					<h2 id="ridingViewTitle">일정</h2> <input type="hidden"
					name="startDate" value="${vo.startDate }">${vo.startDate }
				...
				<ul id="eList">
					...
					<c:if test="${ vo.nickname eq nickName}">
					...
					<c:forEach var="vo" items="${lst2 }">
						...
						<c:if test="${vo.nickname eq nickName}">
							<li><input type="button" id="ridingStateUpdateBtn"
								onclick="ridingStateUpdate()" value="승낙하기"> <a
								href="/riding/ridingStateOk?nickname=${vo.nickname}">승낙하기</a></li>
						</c:if>
					</c:forEach>
				</ul>
				<ul>
					<li>ridingNo ${ridingNo }&nbsp;vo.ridingNo ${vo.ridingNo }&nbsp;vo.userScore
						${vo.userScore }&nbsp;${vo.ridingMemberNo }</li>
				</ul>
				<br>
				<br>
				<br>
				...
				<c:if test="${userId == vo.nickname }">
					<div id="viewBTN">
						<button id="delBtn" onclick="javascript:ridingViewDel()">글 삭제</button>
						<button id="editBtn" onclick="javascript:ridingViewEdit()">글 수정</button>
					</div>
